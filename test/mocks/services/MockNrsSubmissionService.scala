...
trait MockNrsSubmissionService extends AnyWordSpecLike with Matchers with OptionValues with MockitoSugar with BeforeAndAfterEach {
  ...
  override def beforeEach(): Unit = {
    super.beforeEach()
    reset(mockNrsSubmissionService)
  }

  def setupMockNrsReceiptSubmission(data: NrsReceiptRequestModel)
                            (response: SubmissionResult): OngoingStubbing[Future[SubmissionResult]] =
    when(
      mockNrsSubmissionService.nrsReceiptSubmission(
