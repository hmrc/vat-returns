    ...
    new QueryStringBindable[VatReturnFilters] {
      override def bind(key: String, params: Map[String, Seq[String]]): Option[Either[String, VatReturnFilters]] = {
        ...
        boundPeriodKey match {
          case Right(Some(periodKey)) => Some(Right(VatReturnFilters(periodKey)))
          case Right(None) => None
          case Left(error) => Some(Left(error))
        }
      }

      override def unbind(key: String, params: VatReturnFilters): String = params.toSeqQueryParams.map {
